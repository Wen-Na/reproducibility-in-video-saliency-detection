...
def similarity(saliencyMap1, saliencyMap2):

	map1 = cv2.resize(saliencyMap1, saliencyMap2.shape[::-1])
	map2 = saliencyMap2.astype(float)

	if map1.any():
		map1 = (map1 - np.min(map1)) / (np.max(map1) - np.min(map1)) # normalize
		map1 = map1 / np.sum(map1)

	if map2.any():
		map2 = (map2 - np.min(map2)) / (np.max(map2) - np.min(map2)) # normalize
